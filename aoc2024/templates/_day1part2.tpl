...
*/}}
{{- define "aoc2024.day1part2" -}}
{{- $list1 := list }}
{{- $list2 := list }}
{{- range $line := regexSplit "\n" . -1 }}
  {{- $splitted := regexSplit "\\s+" . -1 }}
  {{- if (le 2 (len $splitted)) }}
    {{- $list1 = (append $list1 (index $splitted 0)) }}
    {{- $list2 = (append $list2 (index $splitted 1)) }}
  {{- end }}
{{- end }}
{{- $sum := 0 }}
{{- range $v1 := $list1 }}
  {{- range $v2 := $list2 }}
    {{- $similarity := 0 }}
    {{- if (eq $v1 $v2) }}
      {{- $similarity = add $similarity $v2 }}
    {{- end }}
    {{- $sum = add $sum $similarity }}
  {{- end }}
{{- end }}
{{- $sum }}
{{- end }}
