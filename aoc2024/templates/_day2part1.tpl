...
{{- define "aoc2024.day2part1" -}}
{{- $count := 0 }}
{{- range $line := regexSplit "\n" . -1 }}
  {{- $splitted := regexSplit "\\s+" . -1 }}
  {{- if ge (len $splitted) 2 }}
    {{- $maxLen := 0 }}
    {{- range $num := $splitted }}
      {{- if gt (len $num) $maxLen }}
      {{- $maxLen := len $num }}
      {{- end }}
    {{- end }}
    {{- $padded := list }}
    {{- range $num := $splitted }}
      {{- $padded = append $padded (printf "%05s" $num)}}
    {{- end }}
    {{- $padded = sortAlpha $padded }}
    {{- $sorted := list }}
    {{- range $num := $padded }}
      {{- $sorted = append $sorted (regexReplaceAll "^0+" $num "") }}
    {{- end }}
    {{- $reversed := reverse $sorted }}
    {{- if eq ($sorted | toString) ($splitted | toString) }}
      {{- $prev := index $sorted 0 }}
      {{- $ok := true }}
      {{- range $curr := rest $sorted }}
        {{- if $ok }}
          {{- $diff := sub (int $curr) (int $prev) }}
          {{- if not (and (ge $diff 1) (le $diff 3)) }}
            {{- $ok = false }}
          {{- end }}
          {{- $prev = $curr }}
        {{- end }}
      ...
      {{- if $ok }}
        {{- $count = add 1 $count }}
      {{- end }}
    {{- else if eq ($reversed | toString) ($splitted | toString) }}
      {{- $prev := index $reversed 0 }}
      {{- $ok := true }}
      {{- range $curr := rest $reversed }}
        {{- if $ok }}
          {{- $diff := sub (int $prev) (int $curr) }}
          {{- if not (and (ge $diff 1) (le $diff 3)) }}
            {{- $ok = false }}
          {{- end }}
          {{- $prev = $curr }}
